...
#define LOOPIVDEP
    ...
    struct AdaptorGeneralBase : adaptor_basics_type {
        ...
        static constexpr geometry_type size2(vector_type const&pt) noexcept {
            ...
            for (dim_type iDim = 0; iDim < nDimension; ++iDim) {
                auto constexpr d = base::point_comp_c(pt, iDim);
                d2 += d * d;
            }
            return d2;
        }
        ...
        static constexpr bool are_boxes_overlapped(box_type const&e1, box_type const&e2, bool e1_must_contain_e2 = true,
                                                   bool fOverlapPtTouchAllowed = false) noexcept {
            auto constexpr e1_contains_e2min = does_box_contain_point(e1, base::box_min_c(e2));
    ...
    class MOctree {
        ...
        static auto constexpr m_NChild = pow_ce(2, nDimension);
        ...
        public:
            ...
            constexpr bool IsChildNodeEnabled(child_id_type idChild) const noexcept {
                ...
                return std::find_if(std::begin(m_Children), std::end(m_Children), [midChild](auto constexpr&kChild) {
    ...
    protected: // Grid functions
        ...
        static constexpr std::tuple<std::array<double, nDimension>, std::array<double, nDimension>> getGridRasterizer(
            ...
            auto constexpr rn_divide = static_cast<double>(n_divide);
            for (dim_type iDimension = 0; iDimension < nDimension; ++iDimension) {
        ...
        constexpr std::array<grid_id_type, nDimension> getGridIdPoint(vector_type const&pe) const noexcept {
            ...
            for (dim_type iDimension = 0; iDimension < nDimension; ++iDimension) {
                auto constexpr local_comp = adaptor_type::point_comp_c(pe, iDimension) - adaptor_type::point_comp_c(
        ...
        constexpr std::array<std::array<grid_id_type, nDimension>, 2> getGridIdBox(box_type const&box) const noexcept {
        ...
        inline Node& createChild(Node&nodeParent, child_id_type iChild, morton_node_id_type_cref kChild) noexcept {
            ...
            if constexpr (std::is_integral_v<geometry_type>) {
                ...
                LOOPIVDEP
                for (dim_type iDimension = 0; iDimension < nDimension; ++iDimension) {
                    auto constexpr fGreater = ((child_id_type{1} << iDimension) & iChild) > 0;
                    adaptor_type::point_comp(adaptor_type::box_min(nodeChild.box), iDimension) =
                            fGreater * (adaptor_type::point_comp_c(adaptor_type::box_max_c(nodeParent.box), iDimension)
                                        + adaptor_type::point_comp_c(
                                            adaptor_type::box_min_c(nodeParent.box), iDimension)) * geometry_type{0.5} +
    ...
    public: // Static aid functions
        ...
        static constexpr size_t EstimateNodeNumber(size_t nElement, depth_type nDepthMax,
            ...
            if ((nDepthMax + 1) * nDimension < 64) {
                ...
                auto constexpr nElementInNode = nElement / nMaxChild;
                if (nElementInNode > nElementMax / 2)
                    return nMaxChild;
            }

            auto constexpr nElementInNodeAvg = static_cast<float>(nElement) / static_cast<float>(nElementMax);
            auto constexpr nDepthEstimated = std::min(
        ...
        static inline depth_type EstimateMaxDepth(size_t nElement, max_element_type nElementMax) noexcept {
            ...
            auto constexpr nLeaf = nElement / nElementMax;
    ...
    public:
        static inline morton_grid_id_type MortonEncode(std::array<grid_id_type, nDimension> const&aidGrid) noexcept {
    ...
    public: // Main service functions
        ...
        void VisitNodes(morton_node_id_type_cref kRoot, fnProcedureUnconditional const&procedure,
                        fnSelector const&selector, fnSelectorUnconditional const&selectorUnconditional) const noexcept {
            struct Search {
        ...
        std::vector<entity_id_type> CollectAllIdInBFS(morton_node_id_type_cref kRoot = GetRootKey()) const noexcept {
            auto ids = std::vector<entity_id_type>();
        ...
        box_type CalculateExtent(morton_node_id_type_cref keyNode) const noexcept {
            ...
            LOOPIVDEP
            for (dim_type iDimension = 0; iDimension < nDimension; ++iDimension)
                aSize[iDimension] = adaptor_type::point_comp_c(ptMaxBoxRoot, iDimension) - adaptor_type::point_comp_c(
                                        ptMinBoxRoot, iDimension);

            auto constexpr nDepth = GetDepth(keyNode);
            auto constexpr nRasterResolution = pow_ce(2, nDepth);
        ...
        morton_node_id_type Find(entity_id_type id) const noexcept {
            auto constexpr it = find_if(this->m_nodes.begin(), this->m_nodes.end(), [id](auto constexpr&keyAndNode) {
    ...
    protected:
        ...
        static constexpr void constructGridIdRec(std::array<std::array<grid_id_type, 3>, nDimension> const&avidGridList,
        ...
        void rangeSearch(box_type const&range, std::span<data_type const> const&vData, double rVolumeRange,
            ...
            auto constexpr rVolumeNode = rVolumeParent / this->m_NChild;
            for (morton_node_id_type_cref keyChild: nodeParent.GetChildren()) {
                auto constexpr&nodeChild = this->GetNode(keyChild);

                auto bOverlap = true;
                for (dim_type iDim = 0; iDim < nDimension && bOverlap; ++iDim) {
                    auto constexpr isUpperNodeInTheDimension = IsValidKey(keyChild & (morton_node_id_type{1} << iDim));
        ...
        bool rangeSearchRoot(box_type const&range, std::span<data_type const> const&vData,
            ...
            auto constexpr nEntity = vData.size();
            ...
            auto constexpr keyRange = this->GetHash(nDepth, idLocationMin);
            ...
            auto constexpr rVolumeNode = this->m_rVolume / static_cast<double>(1 << (nDimension * nDepth));

            auto constexpr nidFoundEstimation = this->m_rVolume < 0.01
                                                    ? 10
                                                    : static_cast<size_t>((rVolumeRange * nEntity) / this->m_rVolume);
            sidFound.reserve(nidFoundEstimation);
            auto constexpr&node = this->GetNode(keyNodeSmallest);
            rangeSearch<data_type, fRangeMustContain, fIdCheck>(range, vData, rVolumeRange, rVolumeNode, node, sidFound,
                                                                idMin);
    ...
    public:
        void CollectAllIdInDFS(morton_grid_id_type_cref keyParent, std::vector<entity_id_type>&vItem) const noexcept {
    ...
    class OrthoTreePoint : public MOctree<nDimension, vector_type, box_type, adaptor_type, geometry_type> {
    protected:
        using base = MOctree<nDimension, vector_type, box_type, adaptor_type, geometry_type>;
    ...
    public: // Create
        ...
        OrthoTreePoint(std::span<vector_type const> const&vpt, depth_type nDepthMax,
        ...
        // Create
        template<typename execution_policy_type = std::execution::unsequenced_policy>
        static void Create(OrthoTreePoint&tree, std::span<vector_type const> const&vpt, depth_type nDepthMaxIn = 0,
                           std::optional<box_type> const&oBoxSpace = std::nullopt,
                           max_element_type nElementMaxInNode = max_element_default) noexcept {
    ...
    private: // K Nearest Neighbor helpers
        ...
        static void createEntityDistance(Node const&node, vector_type const&pt, std::span<vector_type const> const&vpt,
